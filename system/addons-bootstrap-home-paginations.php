...
function spectre_pagination($pages = '', $range = 2) {
    $showitems = ($range * 2) + 1;
    global $paged;
    if (empty($paged))
        $paged = 1;
    ...
    if (1 != $pages) {
        ...
        echo '<li class="page-item disabled hidden-md-down d-none d-lg-block"><span class="page-link">' . __('Halaman', 'spectre') . ' ' . $paged . ' ' . __('Dari', 'spectre') . ' ' . $pages . '</span></li>';
        ...
        echo '</ul>';
        echo '</nav>';
        echo '<div class="pagination-info mb-5 text-center">[ <span class="text-muted">' . __('Halaman', 'spectre') .'</span> '.$paged.' <span class="text-muted">'.__('Dari', 'spectre').'</span> '.$pages.' ]</div>';	 	
    }
}
